...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const enrichedData = req.body.data.map((event: any) => {
      ...
      const eventId = event.event_id || `evt_online_fallback_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
    ...
    console.error("Erro interno no servidor CAPI (.online):", err);
